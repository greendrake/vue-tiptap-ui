...
// Separate button-specific attrs from other attrs
...
const formatShortcutKey = (key: string, isMacOs: boolean): string => {
...
const shortcuts = computed(() => {
    if (!props.shortcutKeys) return []

    return props.shortcutKeys
        .split('-')
        .map(key => key.trim())
        .map(key => formatShortcutKey(key, isMac.value))
})
